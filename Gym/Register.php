  ...
  //check if the page is being invoked after register form data has been submitted
		...
		if($row = $resultt -> fetch()){
			...
			if($emailErr == "" && $nameErr == "" && $passwordErr == "" ){
			    ...
			    if($userType == 'Normal'){			    //insert the userID in the appropriate table table
			   		 $mInsert = "INSERT INTO member (userID) VALUES('$uid')";
			    }else{
			    	if($userType == 'Coach'){
			    		$mInsert = "INSERT INTO coach (userID) VALUES('$uid')";
			    	}
			    	else{
			    		$mInsert = "INSERT INTO administrator (userID) VALUES('$uid')";
			    	}
			    }
			  	$conn->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
			    $Result2 = $conn->exec($mInsert) ;
			    ...
			    {	
				    ...
				    if($userType == 'Normal'){			    //insert the userID in the appropriate table table
			   			 header("Location: session_Choose.php");
				    }else{
				    	if($userType == 'Coach'){
  ...
  <div class="row">
    <div class="col-sm-6">
		...
		<form class="form-horizontal" method="POST" action="<?php echo $_SERVER["PHP_SELF"];?>" >
		  ...
		  <div class="form-group">
		    <div class="col-sm-offset-2 col-sm-10">
		     	<a href="session_Choose.php"><button type="submit" class="btn btn-primary">Next</button></a>
		    </div>
		  </div>
